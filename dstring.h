...
/*------------------------------------------------------------------------------------------------
 * to allocate space for a new string, you must pass an initialized pointer, since the allocation
 * is for the buffer pointer, no for the object itself. you have to use dstring_error(dstring*)
 * to obtain the error message string, and dstring_error_ds(dstring*) to obtain a dstring object.
 * Strings are safely treated, using '\0' byte as ending and taking care on it for you, see the
 * documentation 
 */
